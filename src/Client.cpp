...
void Client::SendCreateCharacter() {
    ...
    // DETERMINISTIC PER-CHARACTER APPEARANCE.
    //
    // Every bot sent the same fixed skinHue/hairId/hairHue/shirtHue/pantsHue
    // (see the CreateCharacterParams defaults in include/uo/builders.h), so
    // every character came out with the same face and the same shirt and
    // pants color -- "new characters always having same face male/female
    // shirt or pants or shoes same color we need diversity" (project owner,
    // 2026-08-29). Appearance must still be STABLE for a given character
    // (the same character looks the same on every login/recreate), so it is
    // derived from identity rather than randomised per run, the same way
    // main.cpp:411-427 derives a character's home city: h = h*131 + c over
    // the lowercased "account.charname". Each attribute below reuses that
    // exact hash shape with its own suffix appended to the identity string,
    // so hair color, shirt color, etc. vary independently of each other
    // instead of moving in lockstep.
    auto hashIdent = [&](const char* suffix) -> usize {
        std::string ident;
        ...
        append(suffix);
        usize h = 0;
        for (char c : ident) h = h * 131 + static_cast<unsigned char>(c);
        return h;
    };

    // Sex. Builders.cpp:181/204 confirms the wire encoding: offset 70 is a
    // single byte, even=male/odd=female, written from CreateCharacterParams
    // ::female. Nothing in this codebase set that field before now -- every
    // bot character was implicitly male -- so it is derived here too,
    // otherwise "same face male/female" could never actually be checked.
    p.female = (hashIdent("sex") % 2) != 0;
    ...
    // like a stray 0x8000 bit tacked onto an otherwise-correct value.
    {
        ...
        p.skinHue = static_cast<u16>(kSkinLow + (hashIdent("skin") % (kSkinHigh - kSkinLow + 1)));
    }
    ...
    // ITEMID_NOTHING), so picking from it is what makes the request stick.
    {
        ...
        for (u16 id : kHairStyles) {
            if (id == 0x2046 && !p.female) continue;
            if (id == 0x2048 && p.female) continue;
            candidates[nCand++] = id;
        }
        p.hairId = candidates[hashIdent("hair_style") % nCand];
    }
    ...
    // plausibly has rather than one only a barber's premium service grants.
    {
        ...
        p.hairHue = static_cast<u16>(kHairHueLow + (hashIdent("hair_hue") % (kHairHueHigh - kHairHueLow + 1)));
    }
    ...
    if (!p.female) {
        ...
        p.beardId = kBeardStyles[hashIdent("beard_style") % (sizeof(kBeardStyles) / sizeof(kBeardStyles[0]))];
        if (p.beardId != 0) {
            ...
            constexpr u16 kHairHueLow = 1102, kHairHueHigh = 1149;
            p.beardHue = static_cast<u16>(kHairHueLow + (hashIdent("beard_hue") % (kHairHueHigh - kHairHueLow + 1)));
        }
    }
    ...
    // reuses the shard's own idiom for clothing color, not a new one.
    {
        ...
        p.shirtHue = static_cast<u16>(kDyeLow + (hashIdent("shirt") % (kDyeHigh - kDyeLow + 1)));
        p.pantsHue = static_cast<u16>(kDyeLow + (hashIdent("pants") % (kDyeHigh - kDyeLow + 1)));
    }

    LogInfo("[0x00] appearance sex=%s skin=0x%04X hair=0x%04X/0x%04X beard=0x%04X/0x%04X shirt=0x%04X pants=0x%04X\n",
            p.female ? "female" : "male", p.skinHue, p.hairId, p.hairHue,
            p.beardId, p.beardHue, p.shirtHue, p.pantsHue);
