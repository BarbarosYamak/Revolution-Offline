...
VendorErrandResult VendorErrand::Tick(Client& client, const Observation& obs) {
    ...
    switch (step_) {
        ...
        case Step::Find: {
            ...
            keeper_ = client.NearestMobileWithTrade(spec_.trade);
            if (keeper_) {
                scans_ = 0;
                step_ = Step::Approach;
                return Working(Wake::Now, 0,
                               Fmt("found a '%s'", spec_.trade));
            }
            ...
            if (++trips_ > spec_.maxTrips) {
                ...
                return Failed(Fmt("no '%s' answered after %d trip(s)",
                                  spec_.trade, trips_ - 1));
            }
            scans_ = 0;
            travelInFlight_ = client.TravelToService(spec_.service, nullptr);
            if (!travelInFlight_) {
                return Working(Wake::AfterDelay, kShortMs,
                               Fmt("no route to a '%s': %s", spec_.trade,
                                   client.TravelFailureText()));
            }
            return Working(Wake::TravelArrives, 0,
                           Fmt("walking to a '%s' (trip %d)", spec_.trade,
                               trips_));
        }
        ...
        case Step::Open: {
            ...
            return Working(Wake::AfterDelay, kAfterAskMs,
                           Fmt("asking the '%s' to show %s", spec_.trade,
                               spec_.what));
        }
        ...
        case Step::Buy: {
            ...
            running_ = false;
            return Failed(Fmt("this '%s' does not stock %s", spec_.trade,
                              spec_.what));
        }
